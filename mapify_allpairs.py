...
class MapifyAllPairs(Transform):
  def transform_stmt(self, stmt):
    """
    Assume that all adverbs occur only at the outer level of bindings, 
    so skip recursive evaluation of expressions
    """
  ...
  def transform_AllPairs(self, expr):
    ...
    closure = self.closure(inner_fn, closure_elts + [y_outer], name = "outer_closure") 
