...
def winmap_avg1d(x, w = 3):
  return parakeet.pmap_1d(avg1d, x, w)
...
def winmap_zeros(x, wx = 3, wy = 3):
  def zero(_):
    return 0
  return parakeet.pmap_2d(zero, x, wx, wy)
...
def winmap_first_elt(x, wx = 3, wy = 3):
  def f(window):
    return window[0,0]
  return parakeet.pmap_2d(f, x, wx, wy)
