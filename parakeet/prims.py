...
class Prim(object):
    def __init__(
        ...
        # table mapping mismatching types i.e. (Int32, Float64)
        # to (Float64, Float64)
        self._upcast_types = {}
    ...
    def _signature_distance(self, types1, types2):
        ...
        for (t1, t2) in zip(types1, types2):
            if t1 != t2:
                assert isinstance(t1, ScalarT), \
                    "Expected scalar type but got %s" % t1
                assert isinstance(t2, ScalarT), \
                    "Expected scalar type but got %s" % t2
    ...
    def expected_input_types(self, arg_types):
        ...
        # by default we just figure out the common type and expect every arg
        # to be of that type
    ...
    def result_type(self, arg_types):
        ...
        if key not in self.type_table:
            raise RuntimeError(
                "Primitive %s doesn't support input types %s, candidates=%s" % (
                    self.name,
                    key,
                    self.type_table))
        else:
