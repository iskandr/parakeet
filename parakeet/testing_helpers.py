...
from dsltools.testing_helpers import eq, expect_eq
...
from .cuda_backend.device_info import has_gpu
...
def expect(fn, args, expected, msg=None, valid_types=None):
    """
    Helper function used for testing, assert that Parakeet evaluates given code
    ...
    for backend in available_backends:
        ...
        if valid_types is not None:
            ...
            if type(result) not in valid_types:
                raise TypeError(
                    "Expected result to have type in %s but got %s" % (
                        valid_types,
                        type(result)))
...
def return_type(fn, input_types):
    ...
    return type_inference.infer_return_type(
        untyped_fundef,
...
def timed_test(
    ...
    print("Parakeet time (with compilation): %0.4f" % parakeet_time_with_comp)
    print("Parakeet time (without compilation): %0.4f" % parakeet_time_no_comp)
    print("Python time: %0.4f" % py_time)
