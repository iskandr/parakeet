...
class InsertCoercions(Transform):
  ...
  def transform_PrimCall(self, expr):
      ...
      upcast_args = [self.coerce_expr(x, t) for (x,t) in zip(expr.args, upcast_types)]
...
def insert_coercions(typed_fundef):
  return InsertCoercions(typed_fundef).apply(copy=False)
